...
//declare type TDeriveMapper = (data : any, ...extraParams : any[]) => any
...
export default class AsService {
  private _subscriber : ASubscriber
  ...
  constructor(data : TData, mapper?: TMapper, source?: AsService) {
    ...
    this._defaultObservable = new AObservable(initialValue);
    this._observables = {}
  ...
  getObs(...params : any[]) : AObservable | undefined {
    if(params.length < 1) {
      ...
      if (ret !== undefined) 
        ret = this._observables[key];
      else {
        ret = new AObservable("idle")
  ...
  async load(...params : any[]) : Promise < any > {
    ...
    const ret = new Promise((resolve, reject) => {
      if (this._source !== undefined) {} else {
        ...
        if (loadValue instanceof(AObservable)) {} else if (loadValue instanceof(AObservable)) {} else if (loadValue instanceof(Promise)) {
          if (obs) 
            obs.next([
              obs.getValue()[0],
              "loading"
  ...
  // derive(map : TMapper) : AsService {   const ret = new AsService(this, map,);
  // return new AsService(this) }
